<!DOCTYPE html>
<html>

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, shrink-to-fit=no">
    <title>Home - Brand</title>
    <link rel="stylesheet" href="assets/bootstrap/css/bootstrap.min.css">
    <link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Lato:300,400,700,300italic,400italic,700italic">
    <link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Abril+Fatface&amp;display=swap">
    <link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Actor&amp;display=swap">
    <link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Fredoka+One&amp;display=swap">
    <link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Lora">
    <link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Roboto:300,400,700&amp;display=swap">
    <link rel="stylesheet" href="assets/fonts/font-awesome.min.css">
    <link rel="stylesheet" href="assets/fonts/simple-line-icons.min.css">
    <link rel="stylesheet" href="assets/css/--mp---Testimonials-Slider.css">
    <link rel="stylesheet" href="assets/css/Article-Clean.css">
    <link rel="stylesheet" href="assets/css/best-carousel-slide.css">
    <link rel="stylesheet" href="assets/css/Black-Navbar.css">
    <link rel="stylesheet" href="assets/css/Bold-BS4-Animated-Back-To-Top.css">
    <link rel="stylesheet" href="assets/css/Bold-BS4-CSS-Image-Slider.css">
    <link rel="stylesheet" href="assets/css/Bold-BS4-Footer-Big-Logo.css">
    <link rel="stylesheet" href="assets/css/Features-Boxed.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/animate.css/3.5.2/animate.min.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/aos/2.3.4/aos.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/slick-carousel/1.6.0/slick-theme.min.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/slick-carousel/1.6.0/slick.min.css">
    <link rel="stylesheet" href="assets/css/Landing-Page---Parallax-Background---Logo-Heading-ButtonGIF.css">
    <link rel="stylesheet" href="assets/css/Landing-Section-with-Call-to-Action-BS4.css">
    <link rel="stylesheet" href="assets/css/responsive-blog-card-slider.css">
    <link rel="stylesheet" href="assets/css/Slick-Slider-2-1.css">
    <link rel="stylesheet" href="assets/css/Slick-Slider-2.css">
    <link rel="stylesheet" href="assets/css/Slider-laptop-1.css">
    <link rel="stylesheet" href="assets/css/Slider-laptop.css">
    <link rel="stylesheet" href="assets/css/sticky-dark-top-nav.css">
    <link rel="stylesheet" href="assets/css/Swipe-Slider-9.css">
    <link rel="stylesheet" href="assets/css/Testimonial-Slider-9-1.css">
    <link rel="stylesheet" href="assets/css/Testimonial-Slider-9.css">
    <link rel="stylesheet" href="assets/css/Testimonials.css">
</head>

<body>
    <section class="text-center bg-light features-icons" style="background: var(--bs-teal);height: 2165.094px;">
        <nav class="navbar navbar-light navbar-expand-md navbar-fixed-top navigation-clean-button" style="background: rgb(34, 34, 34);border-radius: 20;border-top-left-radius: 20;border-top-right-radius: 20;border-bottom-right-radius: 20;border-bottom-left-radius: 20;border-style: none;padding-top: 0;padding-bottom: 10px;margin-top: -113px;margin-bottom: 33px;">
            <div class="container"><button data-bs-toggle="collapse" class="navbar-toggler" data-bs-target="#navcol-2"><span class="visually-hidden">Toggle navigation</span><span class="navbar-toggler-icon"></span></button>
                <div><a class="navbar-brand" href="#"><span style="box-shadow: 0px 0px rgb(255,255,255);">AGNORIA</span> </a></div>
                <div class="collapse navbar-collapse" id="navcol-2" style="color: rgb(255,255,255);">
                    <ul class="navbar-nav nav-right">
                        <li class="nav-item"><a class="nav-link active" href="LandingPage.html" style="color: rgba(224,217,217,0.9);">Home </a></li>
                        <li class="nav-item dropdown"><a class="dropdown-toggle nav-link" aria-expanded="false" data-bs-toggle="dropdown" href="#" style="color: rgba(224,217,217,0.9);">Catalog</a>
                            <div class="dropdown-menu"><a class="dropdown-item" href="index.php" style="color: rgb(255,255,255);">Mini PC</a><a class="dropdown-item" href="index.php" style="color: rgb(255,255,255);">Single-Board Computers</a></div>
                        </li>
                        <li class="nav-item"><a class="nav-link" href="#" style="color: var(--bs-gray-500);">PC Builder</a></li>
                        <li class="nav-item"><a class="nav-link" href="About.php" style="color: rgba(224,217,217,0.9);">About </a></li>
                        <li class="nav-item"><a class="nav-link" href="FAQ.php" style="color: rgba(224,217,217,0.9);">FAQs</a></li>
                        <li class="nav-item"><a class="nav-link" href="ContactUs.html" style="color: rgba(224,217,217,0.9);">Contact</a></li>
                    </ul>
                    <p class="ms-auto navbar-text actions"><a class="login" href="UserLogin.php" style="color: rgba(224,217,217,0.9);">Log In</a> <a class="btn btn-light action-button" role="button" href="UserRegister.php" style="color: rgba(0,0,0,0.9);background: var(--bs-gray-200);border-radius: 10px;border-style: solid;border-color: rgba(0,0,0,0.9);font-size: 16px;padding: 5px 8px;">Sign Up</a></p>
                </div>
            </div>
        </nav>
        <div class="container">
            <div class="row">
                <div class="col" style="height: 1250px;">
                    <section class="article-clean">
                        <div class="container" style="height: 1932.58px;">
                            <div class="row" style="height: 1814.58px;">
                                <div class="col-lg-10 col-xl-8 offset-lg-1 offset-xl-2" style="height: 1088.59px;">
                                    <div class="intro">
                                        <h1 class="text-center"><strong>Brand Promos</strong></h1>
                                        <p class="text-center"><span class="by">by</span> <a href="#">AGNORIA DiST</a><span class="date">April 24, 2022</span></p><img class="img-fluid" src="assets/img/FireCuda_1920x700px_c.jpg">
                                    </div>
                                    <div class="text">
                                        <p>Welcome dear customer! We are introducing you the latest models of <strong>SEAGATE </strong>brand! With exclusive promos that you can grab. Get the latest <strong>FIRECUDA </strong>SSD of 240G, 500G, 1TB, and 2TB variant with 3 YEARS of <strong>DATA RECOVERY </strong>services and a <strong>5</strong> years limited warranty!</p>
                                        <p style="font-size: 14px;">SSD specification: We offer NVMe SSD, 2.5i, or SATA 3.0 standard SSD. Prices may vary depending on the storage size. Promo may last until May 30, 2022.</p>
                                        <h2></h2>
                                        <p>_______________________________________________________________________________</p>
                                        <figure class="figure d-block"><img class="figure-img" src="assets/img/AMD-Windows-11-Raffle-1920-x-700-1.jpg">
                                            <figcaption class="figure-caption">RYZEN EXCLUSIVE PROMO</figcaption>
                                        </figure>
                                        <p>Grab the new latest <strong>AMD RYZEN 5000 </strong>series processors for your gaming and work needs! With the new promo, you can get a free <strong>HALO INFINITE </strong>REDEEM CODE upon purchasing any RYZEN 5000 series model!</p>
                                    </div>
                                    <p style="font-size: 14px;margin-bottom: 33px;">Promo is applicable in any 5000 model including M types. Valid until May 30th, 2022.</p>
                                </div>
                                <div class="col-lg-10 col-xl-8 offset-lg-1 offset-xl-2" style="height: 796.5899999999998px;margin-top: 38px;">
                                    <div class="intro">
                                        <p class="text-center"> </p>
                                    </div>
                                    <div class="text">
                                        <h2></h2>
                                        <p>_______________________________________________________________________________</p>
                                        <figure class="figure d-block"><img class="figure-img" src="assets/img/maniwp.jpg">
                                            <figcaption class="figure-caption">RYZEN EXCLUSIVE PROMO</figcaption>
                                        </figure>
                                        <p>Grab the new latest <strong>AMD RYZEN 5000 </strong>series processors for your gaming and work needs! With the new promo, you can get the new MICROSOFT WINDOWS 11 for free!</p>
                                    </div>
                                    <p style="font-size: 14px;">Promo is applicable in any 5000 model including M types. Valid until May 30th, 2022.</p><button class="btn btn-primary" type="button" style="color: var(--bs-orange);font-size: 23px;margin-top: 18px;width: 189px;box-shadow: 0px 0px 4px;" h=""><strong>SHOP NOW</strong>!</button>
                                </div>
                            </div>
                        </div>
                    </section>
                </div>
            </div>
        </div>
    </section>
    <section style="height: 558px;background: #2f2f2f;margin-top: -114px;opacity: 1;filter: blur(0px);">
        <div class="container" style="height: 484px;margin-top: 51px;padding-top: 83px;">
            <div class="blog-slider">
                <div class="blog-slider__wrp swiper-wrapper">
                    <div class="blog-slider__item swiper-slide">
                        <div class="blog-slider__img"><img src="https://res.cloudinary.com/muhammederdem/image/upload/v1535759872/kuldar-kalvik-799168-unsplash.jpg"></div>
                        <div class="blog-slider__content"><span class="blog-slider__code">24 04 April 2022</span>
                            <div class="blog-slider__title">
                                <p>Fresh Management</p>
                            </div>
                            <div class="blog-slider__text">
                                <p>We have established some systematic rules for our new employees and provide enough equipments!</p>
                            </div><a class="blog-slider__button" href="#">TBA</a>
                        </div>
                    </div>
                    <div class="blog-slider__item swiper-slide">
                        <div class="blog-slider__img"><img src="https://res.cloudinary.com/muhammederdem/image/upload/v1535759872/kuldar-kalvik-799168-unsplash.jpg"></div>
                        <div class="blog-slider__content"><span class="blog-slider__code">24 04 April 2022</span>
                            <div class="blog-slider__title">
                                <p>New arrivals of NUC and ITX pre-built models</p>
                            </div>
                            <div class="blog-slider__text">
                                <p>ASUS NUCS are now available! ITX models for Dell and Acer are here!</p>
                            </div><a class="blog-slider__button" href="#">TBA</a>
                        </div>
                    </div>
                    <div class="blog-slider__item swiper-slide">
                        <div class="blog-slider__img"><img src="https://res.cloudinary.com/muhammederdem/image/upload/v1535759872/kuldar-kalvik-799168-unsplash.jpg"></div>
                        <div class="blog-slider__content"><span class="blog-slider__code">24 04 April 2022</span>
                            <div class="blog-slider__title">
                                <p>Delivery method still on experimental</p>
                            </div>
                            <div class="blog-slider__text">
                                <p>Don't worry, we are enhancing your experience for delivery services!</p>
                            </div><a class="blog-slider__button" href="#">TBA</a>
                        </div>
                    </div>
                </div>
                <div class="blog-slider__pagination"></div>
            </div>
        </div>
    </section>
    <section class="showcase">
        <div class="container-fluid p-0">
            <div class="row g-0">
                <div class="col">
                    <div class="x_001_laptop_main_wrap"></div>
                </div>
            </div>
            <div class="row g-0">
                <div class="col">
                    <section class="features-boxed"></section>
                </div>
            </div>
        </div>
    </section>
    <section class="text-center text-white call-to-action" style="background:url(&quot;assets/img/itx-end.jpg&quot;) no-repeat center center;background-size:cover;">
        <div class="overlay"></div>
        <div class="container">
            <div class="row">
                <div class="col-xl-9 mx-auto position-relative">
                    <h2 class="mb-4">Want to stay on news? Subscribe to our newsletter!</h2>
                </div>
                <div class="col-md-10 col-lg-8 col-xl-7 mx-auto position-relative">
                    <form>
                        <div class="row">
                            <div class="col-12 col-md-9 mb-2 mb-md-0"><input class="form-control form-control-lg" type="email" placeholder="Enter your email..."></div>
                            <div class="col-12 col-md-3"><button class="btn btn-primary btn-lg" type="submit">Inform me!</button></div>
                        </div>
                    </form>
                </div>
            </div>
        </div>
    </section>
    <footer class="bg-light footer">
        <div style="height: 132px;margin-top: -64px;">
            <footer id="myFooter">
                <div class="container-fluid">
                    <div class="row text-center">
                        <div class="col-12 col-sm-6 col-md-3">
                            <h1 class="logo" style="margin-top:30px;"><a href="#" style="font-size: 37px;font-weight: bold;">&nbsp;AGNORIA</a></h1>
                        </div>
                        <div class="col-12 col-sm-6 col-md-2">
                            <h5>Get started</h5>
                            <ul>
                                <li><a href="LandingPage.php">Home</a></li>
                                <li><a href="index.php">Catalog<br></a></li>
                                <li><a href="UserRegister.php">Sign Up</a></li>
                                <li><a href="PromoPage.php">Promos/Other</a></li>
                            </ul>
                        </div>
                        <div class="col-12 col-sm-6 col-md-2">
                            <h5>Our Company</h5>
                            <ul>
                                <li><a href="About.php">About Us</a></li>
                                <li><a href="#"></a></li>
                                <li><a href="LandingPage.php">Reviews</a></li>
                                <li><a href="ContactUs.php">Contacts</a></li>
                            </ul>
                        </div>
                        <div class="col-sm-6 col-md-2">
                            <h5>Support</h5>
                            <ul>
                                <li><a href="FAQ.php">FAQ</a></li>
                                <li><a href="ConctactUs.php">Help Desk<br></a></li>
                                <li>Forums[TBA]</li>
                                <li><a href="#">External Links</a></li>
                            </ul>
                        </div>
                        <div class="col-md-3 social-networks">
                            <div></div><a class="facebook" href="#"><i class="fa fa-facebook"></i></a><a class="twitter" href="#"><i class="fa fa-twitter"></i></a><a class="google" href="#"><i class="fa fa-google-plus"></i></a><a class="linkedin" href="#"><i class="fa fa-linkedin"></i></a><button class="btn btn-primary" style="margin-top:20px;" type="button">Contact us</button>
                        </div>
                    </div>
                    <div class="row footer-copyright">
                        <div class="col">
                            <p>© 2022 Open-design - by: AGNORIA. DIST. co</p>
                        </div>
                    </div>
                </div>
            </footer>
        </div>
    </footer><a class="cd-top js-cd-top cd-top--fade-out cd-top--show" style="background-image: url(&quot;assets/img/cd-top-arrow.svg&quot;);border-color: var(--bs-black);" href="#0">Top</a>
    <script src="assets/bootstrap/js/bootstrap.min.js"></script>
    <script src="assets/js/bs-init.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/aos/2.3.4/aos.js"></script>
    <script src="assets/js/--mp---Testimonials-Slider.js"></script>
    <script src="assets/js/Bold-BS4-Animated-Back-To-Top.js"></script>
    <script src="https://use.fontawesome.com/1744f3f671.js"></script>
    <script src="assets/js/responsive-blog-card-slider-1.js"></script>
    <script src="assets/js/responsive-blog-card-slider.js"></script>
    <script src="assets/js/Slick-Slider-2.js"></script>
    <script src="assets/js/Slider-laptop.js"></script>
    <script src="assets/js/Swipe-Slider-9.js"></script>
</body>

</html>
